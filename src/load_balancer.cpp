...
LoadBalancer::LoadBalancer(int lb_id,
    ...
    : lb_id_(lb_id),
      ...
      per_fp_counts_(num_fps_, 0) {
      // NOTE: per_fp_counts_ must use num_fps_ (already initialized),
      // NOT fp_queues.size() — fp_queues was already moved by fp_queues_ init.
}
...
void LoadBalancer::run() {
    while (running_) {
        ...
        // Guard: cannot dispatch if we have no FPs
        if (num_fps_ <= 0 || fp_queues_.empty()) {
            continue;
        }
...
LoadBalancer& LBManager::getLBForPacket(const FiveTuple& tuple) {
    ...
    FiveTupleHash hasher;
    size_t hash = hasher(tuple);
    size_t count = lbs_.size();
    if (count == 0) {
        // Should never happen if properly initialized; fallback to LB 0
        // Caller must ensure LBManager has at least 1 LB.
        static LoadBalancer fallback(0, {}, 0);
        return fallback;
    }
    int lb_index = hash % count;
