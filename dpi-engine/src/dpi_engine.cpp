...
void DPIEngine::readerThreadFunc(const std::string& input_file) {
    ...
    while (reader.readNextPacket(raw)) {
        ...
        // Nullify payload_data — raw pointer into job.data becomes dangling
        // after move. FP threads recalculate from job.data.data() + offset.
...
PacketJob DPIEngine::createPacketJob(const PacketAnalyzer::RawPacket& raw,
    ...
    if (job.data.size() >= job.transport_offset + tcp_header_len) {
        job.payload_offset = job.transport_offset + tcp_header_len;
    } else {
        return job;  // prevent crash
    }
} else if (parsed.has_udp) {
    if (job.data.size() >= job.transport_offset + 8) {
...
void DPIEngine::injectLivePacket(const uint8_t* data, size_t length,
    ...
    if (!lb_manager_ || !fp_manager_) return;
    if (!data || length == 0) return;

    // 14 bytes Ethernet + 20 bytes minimum IP header
    if (length < 34 || length > 65535) return;

    PacketAnalyzer::RawPacket raw;

    raw.header.incl_len = static_cast<uint32_t>(length);
    raw.header.orig_len = static_cast<uint32_t>(length);
    raw.header.ts_sec = 0;
    ...
    // Use same static parser style as file mode
    if (!PacketAnalyzer::PacketParser::parse(raw, parsed)) {
        return;
    }

    // Only process IP packets with TCP/UDP (same as file mode)
    if (!parsed.has_ip || (!parsed.has_tcp && !parsed.has_udp)) {
        return;
    }
    ...
    // becomes dangling after the job is moved. FP threads recalculate it
    // from job.data.data() + job.payload_offset, so this field is not needed.
    job.payload_data = nullptr;

    // Update stats exactly like file mode
    stats_.total_packets.fetch_add(1, std::memory_order_relaxed);
    stats_.total_bytes.fetch_add(raw.data.size(), std::memory_order_relaxed);

    if (parsed.has_tcp)
        stats_.tcp_packets.fetch_add(1, std::memory_order_relaxed);
    else if (parsed.has_udp)
        stats_.udp_packets.fetch_add(1, std::memory_order_relaxed);

    // Re-check running after potentially slow work above
    if (!running_.load(std::memory_order_acquire)) return;
    if (!lb_manager_ || lb_manager_->getNumLBs() <= 0) return;

    LoadBalancer& lb = lb_manager_->getLBForPacket(job.tuple);
    lb.getInputQueue().push(std::move(job));

    // Print live status every 2000 packets
    uint64_t count = stats_.total_packets.load(std::memory_order_relaxed);
    if (count % 10 == 0) {
        printStatus();
    }
}
